...
async def select_square(callback: CallbackQuery):
    ...
    async with locked_game(game_type, int(game_id_text)):
        await select_square_locked(callback, game_type, int(game_id_text), square)


async def select_square_locked(callback: CallbackQuery, game_type: str, game_id: int, square: str):
    ...
    if user_color != game.turn:
        await answer_callback(callback, "Hozir sizning navbatingiz emas.", show_alert=True)
        return
    ...
    if game_type == "chess":
        ...
        if not piece or ("white" if piece.color == chess.WHITE else "black") != user_color:
            await answer_callback(callback, "O'z donangizni tanlang.", show_alert=True)
            return
    else:
        board = dict(game.board_state)
        piece = board.get(square)
        if game.forced_square and square != game.forced_square:
            await answer_callback(callback, "Ko'p urishni shu dona bilan davom ettiring.", show_alert=True)
            return
        ...
        if not checkers.legal_moves_for_square(board, user_color, square, game.forced_square):
            await answer_callback(callback, "Bu dona yura olmaydi.", show_alert=True)
            return
...
async def move_piece(callback: CallbackQuery):
    ...
    async with locked_game(game_type, int(game_id_text)):
        await move_piece_locked(callback, game_type, int(game_id_text), from_square, to_square)


async def move_piece_locked(callback: CallbackQuery, game_type: str, game_id: int, from_square: str, to_square: str):
    game = await load_game(game_type, game_id)
    if game.status not in ACTIVE_STATUSES:
        await answer_callback(callback, "O'yin tugagan.", show_alert=True)
        return
    ...
    if not apply_clock(game, user_color):
        ...
        await answer_callback(callback, "Vaqt tugadi.", show_alert=True)
        return
...
async def handle_chess_move(callback: CallbackQuery, game: ChessGame, user_color: str, from_square: str, to_square: str):
    ...
    if move not in board.legal_moves:
        await answer_callback(callback, "Bu yurish mumkin emas.", show_alert=True)
        return
